...
class SearchScreen extends Component {
    ...
    constructor(props) {
        super(props);
        this.state = {
          coinsData: this.props.reduxData,
      ...
      async componentDidMount() {
        if (this.state.coinsData.length > 0) {
            this.setState({dataToRender: this.state.coinsData, isLoading: false})
        } else {
    ...
    getCoinsData = async (currency) => {
        ...
        .then((json) => this.setState({ coinsData: json, dataToRender: json }))
    ...
    renderSort = () => {
        ...
        return (
            <View style={{flexDirection: 'row', justifyContent: 'space-between', width: ScreenWidth-50, marginBottom: 20, paddingLeft: 30}}>
  ...
  render() {
    ...
    let ScreenWidth = Dimensions.get("window").width;
    const { search, navigationData } = this.state;

    return (
        <View>
            <View style={[styles.containerMain, { height: ScreenHeight, width: ScreenWidth }]}>
            <View style={[styles.headerContainer, {paddingTop: 200, flexDirection: 'row',justifyContent: 'flex-start', paddingLeft: 20 }]}>
              <TouchableOpacity style={{marginRight: 15}} onPress={() => NavigationService.navigate(navigationConstants.COIN_SCREEN, {element: navigationData})}>
                  <Image style={{height: 24, width: 28}} source={arrow} />
              </TouchableOpacity>

              <View style={{position: 'absolute', alignSelf: 'center', bottom: 0, left: (ScreenWidth/2)-30 }}>
                <Text style={styles.searchLabel}>Search</Text>
              </View>
            </View>

            <View style={{width: ScreenWidth-30, paddingTop: 15, marginBottom: 35, paddingLeft: 20}}>
                <SearchBar
            ...
            <View  style={{ paddingBottom: 150, height: ScreenHeight }} >
